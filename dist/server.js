...
import { createMemoryInputSchema, deleteMemoryInputSchema, getMemoryInputSchema, listMemoryInputSchema, memoryListSchema, memorySchema, updateMemoryInputSchema, } from "./schemas.js";
...
function toJsonContent(payload, message) {
    const lines = [];
    if (message) {
...
export function createContextMemoryServer(options = {}) {
    ...
    server.registerTool("memory.add", {
...
export async function startContextMemoryServer(options = {}) {
    ...
    let resolveWait;
    let rejectWait;
    const waitUntilClosed = new Promise((resolve, reject) => {
        resolveWait = resolve;
        rejectWait = reject;
    });
